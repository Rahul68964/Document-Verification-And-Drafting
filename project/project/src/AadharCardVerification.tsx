...
export default function DocumentVerification() {
  ...
  const extractDetails = (details: string) => {
    ...
    let idMatch;
    if (data?.type === "Pan Card") {
      idMatch = cleanDetails.match(/Identification Number \(PAN\):\s*([\w\d]+)/);
    } else if (data?.type === "Aadhaar Card") {
      idMatch = cleanDetails.match(/Aadhaar Number:\s*([\d]+)/);
    }
    
    const idNumber = idMatch ? idMatch[1] : "Not found";
        ...
        {showModal && data && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
            <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
              ...
              <div className="mt-4">
                {data.type === "Pan Card" || data.type === "Aadhar Card" ? (
                  <>
                    <p className="text-lg font-bold text-gray-800">Name:</p>
                    <p className="text-xl font-semibold text-blue-700">{extractDetails(data.details).name}</p>
                    <p className="text-lg font-bold text-gray-800 mt-2">{data.type} Number:</p>
                    <p className="text-xl font-semibold text-blue-700">{extractDetails(data.details).idNumber}</p>
                  </>
                ) : (
                  <p className="text-lg font-semibold text-red-600 text-center">This is not a valid document</p>
                )}
              </div>
